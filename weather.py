...
opad = (
    "Rain", "Snow", "Będzie padać", "Nie będzie padać", "Padało", "Nie padało"
    )
...
def unix_na_ludzki(data_unix):
    data_ludzka = datetime.datetime.utcfromtimestamp(int(data_unix)).strftime(
        "%Y-%m-%d"
    )
    return data_ludzka
...
def ludzki_na_unix(data_ludzka):
    data_unix = int(
        datetime.datetime.strptime(
            f"{data_ludzka} 11:00:00", "%Y-%m-%d %H:%M:%S"
        ).timestamp()
    )
    return data_unix
...
def czytaj_16dni():
    ...
    for dzien in dane["list"]:
        ...
        historia[data] = (
            opad[2] if dzien["weather"][0]["main"] in opad else opad[3]
            )
    return historia
...
with open("pogoda.txt", "w") as pogoda_log:
    json.dump(
        historia, pogoda_log, sort_keys=True, indent=4, ensure_ascii=False
        )
